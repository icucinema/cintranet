...
import datetime
from ticketing import models as m
...
def build_agg_data_for_show_week(film, show_week):
    ...
    for showing in showings:
        ...
        ticket_counts = showing.tickets().exclude(status='void').values('ticket_type__name', 'ticket_type__box_office_return_price').annotate(count=Count('id'))
        refunded_ticket_counts = showing.tickets().filter(status='refunded').values('ticket_type__name', 'ticket_type__box_office_return_price').annotate(count=Count('id'))
...
def generate_pdf(output_to, film, show_week, completer, completed_on, final_agg_data):
    # generate the pdf
    ...
    create_pdfdoc(output_to, story, "DRAFT")

if __name__ == '__main__':
    film = m.Film.objects.get(name='Test Film A')
    show_week = get_show_week(film.showings.first().start_time)
    ...
    generate_pdf(buff, film, show_week, completer, completed_on, build_data_structures(build_agg_data_for_show_week(film, film.showings.first().start_time)))
