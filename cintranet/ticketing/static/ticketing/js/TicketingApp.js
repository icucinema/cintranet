...
app.controller('FilmsCtrl', function($rootScope, $scope, $routeParams, $location, Restangular) {
	...
	$scope.addFilm = {
		...
		go: function() {
			...
			films.customGETLIST('search_tmdb', {'query': st, 'with_images': true, 'with_videos': true}).then(function(res) {
...
app.controller('FilmCtrl', function($rootScope, $scope, $routeParams, $location, Restangular, djurl) {
	...
	film.get({'with_images': true, 'with_videos': true}).then(function(res) {
		$scope.loading = false;
		$scope.data = res;
	});
