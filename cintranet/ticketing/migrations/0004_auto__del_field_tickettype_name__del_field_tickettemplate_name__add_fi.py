...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'TicketType.name'
        db.delete_column(u'ticketing_tickettype', 'name')

        # Deleting field 'TicketTemplate.name'
        db.delete_column(u'ticketing_tickettemplate', 'name')

        # Adding field 'BaseTicketInfo.name'
        db.add_column(u'ticketing_baseticketinfo', 'name',
                      self.gf('django.db.models.fields.CharField')(default='Default Name - %s', max_length=128),
                      keep_default=False)


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'TicketType.name'
        raise RuntimeError("Cannot reverse this migration. 'TicketType.name' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'TicketTemplate.name'
        raise RuntimeError("Cannot reverse this migration. 'TicketTemplate.name' and its values cannot be restored.")
        # Deleting field 'BaseTicketInfo.name'
        db.delete_column(u'ticketing_baseticketinfo', 'name')
